 ...
 */
free_area_t free_area;  //free_area_t 是一个结构体，该结构体通过一个双向链表 free_list 和一个整数 nr_free 来实现对空闲页面的管理
...
default_init_memmap(struct Page *base, size_t n) {
    assert(n > 0);
    struct Page *p = base;  //base：指向物理内存的起始页，这段内存块的起始地址
    for (; p != base + n; p ++) {
        assert(PageReserved(p));
        p->flags = p->property = 0;
        set_page_ref(p, 0); //页面当前没有任何引用或使用
    }
    base->property = n;  //将 base 页面块的 property 属性设置为 n，表示这块内存包含 n 个连续页面。
    SetPageProperty(base);  //PG_property：这是表示页面属性的一个标志，通常用来表示一个页面是否是一个连续内存块的起始页面。如果该页面是一个空闲块的头页，PG_property 位将被设置为 1。
    nr_free += n;
    if (list_empty(&free_list)) {
        ...
        while ((le = list_next(le)) != &free_list) {
            ...
            if (base < page) { //base 是需要插入的页面指针，page 是当前节点指向的页面指针
                list_add_before(le, &(base->page_link));  //在当前节点 le 之前插入 base
                break;
...
default_alloc_pages(size_t n) {  //实现的是内存页面的分配，它使用的是First Fit 分配算法。该函数会在空闲页面列表中找到第一个满足所需页面数量的块，进行分配，并更新页面属性
    ...
    if (page != NULL) {  //找到了合适的页面块
        list_entry_t* prev = list_prev(&(page->page_link));  //当前页面的前一个节点
        list_del(&(page->page_link));  //将当前页面块从空闲链表中移除
        if (page->property > n) {
...
default_free_pages(struct Page *base, size_t n) {  //default_free_pages 函数的主要作用是将之前分配的页面释放回自由页面链表中，并尝试将相邻的空闲页面合并成更大的内存块，以减少内存碎片
    ...
    //尝试合并相邻的页面块
    list_entry_t* le = list_prev(&(base->page_link)); //首先检查 base 前面的页面块 p，如果 p 和 base 是相邻的（即 p + p->property == base），那么将 p 和 base 合并成一个更大的页面块。合并后，p 的 property 会增加 base 的 property，并且 base 将从链表中删除。
    if (le != &free_list) {
    ...
    le = list_next(&(base->page_link));  //接着检查 base 后面的页面块 p
    if (le != &free_list) {
        p = le2page(le, page_link);
