...
//码定义了一些与内存管理单元（Memory Management Unit, MMU）相关的宏和常量，用于操作线性地址和物理地址，以及页表项的字段
...
// construct linear address from indexes and offset：根据 PDX1、PDX0、PTX 和 PGOFF 的值构造一个线性地址
#define PGADDR(d1, d0, t, o) ((uintptr_t)((d1) << PDX1SHIFT | (d0) << PDX0SHIFT | (t) << PTXSHIFT | (o)))

// address in page table or page directory entry
#define PTE_ADDR(pte)   (((uintptr_t)(pte) & ~0x3FF) << (PTXSHIFT - PTE_PPN_SHIFT))  //用于从页表项（PTE，Page Table Entry）中提取物理页地址。页表项的目的是将虚拟地址映射到物理地址
#define PDE_ADDR(pde)   PTE_ADDR(pde)  //于从页目录项（PDE，Page Directory Entry）中提取物理地址。页目录项的作用是指向下一级页表，因此 PDE 中的物理地址指向的是下一级页表的物理页帧号
